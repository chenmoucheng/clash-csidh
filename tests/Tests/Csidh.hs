...
type P'' = 337636219168272189589673161639620127053
...
type StoreN = PrimeField.MontgomeryN.T 340282366920938463463374607431768211456 4 (W.T (C.Unsigned 257)) -- PrimeField.MontgomeryN only works with unsigned store types
...
prop_scalarMultiplicationIsHomomorphicN :: H.Property
prop_scalarMultiplicationIsHomomorphicN =
  H.property $ do
    n <- H.forAll . Gen.integral $ Range.linear 0 30
    m <- H.forAll . Gen.integral $ Range.linear 0 30
    x <- H.forAll $ PrimeField.gen @Csidh.P @P''
    z <- H.forAll $ PrimeField.gen @Csidh.P @P''
    H.assert $ Csidh.prop_scalarMultiplicationIsHomomorphic @Csidh.Scalar @(Csidh.K P'' StoreN) n m x z

prop_groupActionCommutesWithStoreMultiplicationN :: H.Property
prop_groupActionCommutesWithStoreMultiplicationN =
  H.property $ do
    i <- H.forAll . Gen.integral $ Range.linear 0 (length Csidh.ells - 1)
    let ell = Csidh.ells C.!! i
    let f x = Csidh.ellTorsionPoint (ell, x, 1, 0)
    ...
    let (xQ, zQ) = f x
    n <- H.forAll . Gen.integral $ Range.linear 1 1000
    H.assert $ Csidh.prop_groupActionCommutesWithScalarMultiplication @Csidh.Scalar @(Csidh.K P'' StoreN) ell (xQ / zQ) n x

prop_groupActionIsCommutativeN :: H.Property
prop_groupActionIsCommutativeN =
  H.property $ do
    skA <- H.forAll Csidh.genkey2
    skB <- H.forAll Csidh.genkey2
    let xPs = C.iterateI (1 +) 1
    H.assert $ Csidh.prop_groupActionIsCommutative @1000 @P'' @StoreN skA skB xPs
